...
This repo used to go by an earlier working name for the initiative. It has been renamed to
...
- **The rename is done on GitHub and locally, but not fully cleaned up.**
  The initiative's GitHub repo was renamed to
  `Hybrid-Solutions-Cloud/homestead-foundry` (old name
  auto-redirects). This folder is a fresh copy of the old
  working tree under its earlier working name, made because
...
- **Nothing has been committed or pushed yet.** `git status` here should
  show the same uncommitted state as when this was copied: a new folder for
  the private planning workspace (untracked), and modified `.claude/agents/foundry-architect.md`,
...
- **Local cleanup still needed (do NOT do silently):**
  - The old working-tree folder under the earlier working name still
  ...
  - A second stale duplicate exists in a parent folder with no hyphens in
    its name - clones the OLD GitHub name, never resolved this session.
  - A third clone under a different, unrelated GitHub org is a dead,
    disconnected clone (its GitHub repo was deleted 2026-07-13) - read-only
    historical reference only, never push/pull it.
