...
The `capacity 1` figures below record the original single-image-model build.
They are not current guidance and must not be copied. Capacity is now set per
...
`GlobalStandard` still bills per token. See
[the deployment guide](../guide/deployment) and
[as-built](../implementation/as-built). Corrected 2026-08-02.
:::
