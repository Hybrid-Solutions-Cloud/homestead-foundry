...
| 7 | **Arc run-command's own execution timeout, and whether a multi-GB model pull can complete inside one invocation.** | The Arc run-command documentation states no timeout value. A hard execution timeout is documented for *Azure VM* run-command, which is a different service, so the figure cannot be assumed to carry over. | Read the `Microsoft.HybridCompute` run-command REST reference for a documented timeout or `timeoutInSeconds` property, then confirm empirically with a long-running no-op before the production automation depends on a large pull inside one invocation. Splitting install and model-pull into two run-commands is the obvious mitigation if a limit exists. |

Unknowns 1, 2, and 5 all resolve from the same short install test. Unknowns 3, 4, and 7 are read-only and resolvable now without installing anything.
...
5. **Then run one throwaway install test on a dedicated, disposable Windows Server 2025 build VM to close unknowns 1, 2, and 5 together.** Scope the test to: provision the MSIX for all users, `foundry service status`, pull `phi-4-mini`, time one fixed prompt, record tokens per second, then uninstall. **This needs the owner's explicit authorization, because it installs software, which this repo's rules gate.** It does not need any Azure resource, any spend, or any Arc onboarding, so it is the cheapest possible way to retire three unknowns at once.

   **Not on a platform host.** A governed environment should be expected to carry a build-environment policy that ranks runners by cost and isolation, reserves Windows build hosts for genuinely Windows-only work, and treats a general-purpose platform host as a host *for* build targets rather than a build target itself. This install is genuinely Windows-only (Appx provisioning plus a Windows service), so it belongs on a dedicated Windows build VM. That is also the better choice on the merits: a disposable VM means a throwaway install leaves no MSIX package, Windows service, or multi-GB model cache on a machine that runs other work. Where the build VM is powered off when idle, drive the test through a wrapper that records the original power state, starts the VM, waits for remote management, runs the script, and restores the original state.

   Two caveats. A build VM is usually provisioned smaller than a platform host, so a tokens-per-second result is a **floor**, not a prediction: fast enough on fewer cores is certainly fast enough on more, but a slow result warrants one re-measurement on a VM sized to the real target before condemning the approach. And a nested-virtualization build VM is likewise GPU-less, so the GPU-less premise holds and the `WinML` path stays correctly out of scope.

   That policy's own rationale is worth noting beyond the test: it prefers isolated local virtualization over cloud run-command for build workloads because Azure VM run-command carries a hard execution timeout, permits one execution at a time, can deadlock its extension, and gives no shell escape hatch when a command hangs. Arc run-command is a different service and those limits should not be assumed to transfer verbatim, but the failure shapes are close enough to matter for a step that pulls multi-GB model files. See UNKNOWN #7: verify Arc run-command's own timeout behaviour before the production automation depends on a large model pull inside one invocation.
...
Local host measurements in Q1 were taken read-only on this repo's platform VM (Windows Server 2025) on 2026-07-25 via `Get-CimInstance` and `Get-Command`. No software was installed and no state was changed.

The build-environment constraints and the Azure VM run-command limitations cited in recommendation 5 come from the operator's private platform standards, consulted 2026-07-25 via the governance MCP that governs this repo. The specific build hosts, addresses, resource groups, and vault secret names are deliberately not reproduced in this public record; only the rule and its rationale are.
