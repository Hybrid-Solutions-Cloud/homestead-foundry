...
The matrix now carries this quota alongside the catalog, joined on `usageName`,
so the comparison answers "where do I have room" rather than only "what exists".
:::

### Q5a. GlobalStandard quota is one subscription-wide pool, not a per-region grant

This was measured after the fact, prompted by the obvious next question: if a
model is throttled in one region, is there a region with more?

**There is not, for `GlobalStandard`.** Quota was read across all 40 regions with
a catalog. On a subscription holding **exactly one account, in East US**, with a
single `DeepSeek-V4-Pro` deployment on `GlobalStandard` at capacity 1,000:

| Reading | Result |
|---|---|
| Regions offering `DeepSeek-V4-Pro` | 33 |
| Regions reporting `GlobalStandard` **1,000 of 1,000 consumed** | **33, all of them** |
| Deployments actually existing outside East US | **none** |

A quota consumed in every region by a deployment that exists in one is not a
per-region grant. **`GlobalStandard` is a single subscription-wide allocation**,
reported identically in every region, which is consistent with what "Global"
means for routing but is not how a regional quota is usually read. Redeploying
the same model elsewhere gains nothing at all.

**The separate allocation is per deployment type.** The same model held **0 of
1,000 on `DataZoneStandard` in nine US regions**. `DeepSeek-V4-Flash` matched the
pattern: 250 of 250 consumed on `GlobalStandard` in all 33 regions, against 0 of
250 free on `DataZoneStandard` in sixteen.

**So the escalation order when a model throttles is: change deployment type,
then change model, then request an increase.** Changing region is the intuitive
first move and it is the one that cannot work.

Across the whole estate, **six models have no unused quota in any region they are
offered in**: `Kimi-K2.7-Code`, `Llama-4-Maverick-17B-128E-Instruct-FP8`,
`Phi-4-reasoning`, and all three `MAI-Image-2.5` variants. The image models
matter here, because this project's worked example runs on them.
...
| `pull-quota.ps1` | Reads this subscription's quota per region, joined on each SKU's `usageName` |
...
node ./scripts/model-matrix/build-matrix.mjs ./.cache/models ./.cache/onprem.json ./docs/public/data/model-matrix.json 2026-08-05 ./.cache/quota
```

The quota argument is optional; without it the matrix still renders availability
and configuration, and simply omits the headroom layer.
...
6. **When a model throttles, change the deployment type, not the region.** `GlobalStandard` quota is one subscription-wide pool; a second SKU carries a separate allocation. Six models on this subscription have no unused quota anywhere, including all three `MAI-Image-2.5` variants.
