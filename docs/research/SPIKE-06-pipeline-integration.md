...
This is the production instance the pattern above was validated against: the repo, tool, brand, and bucket structure (anonymized to Brand A and Brand B), the live-state proof, and the concrete file list. Everything here is one instantiation of the generic Findings; the pattern is what carries forward, this appendix is the evidence.

**The pipeline and repos.** The consumer repos are Brand B's publish pipeline and Brand A's publish pipeline, each carrying a full `tools/` pipeline (`tts.mjs`, `stitch.mjs`, `publish.mjs`, `r2-upload.mjs`, plus `tools/lib/md.mjs`), a `brands/` folder with a per-brand `brands/<brand>/brand.json`, and a `tools/.state/<brand>.json`. Object storage is Cloudflare R2, uploaded via the `wrangler r2 object put` CLI. The two brands served are Brand A and Brand B, each published from its own repo.

**Q1 proof (line-cited).** `synthesizeChapter(chapter, voice, workDir, { key, region, onProgress })` iterates speakable blocks and calls `synthesizeBlock(text, voice, { key, region })` (`tools/tts.mjs:19`, `:34`), auth `sdk.SpeechConfig.fromSubscription(key, region)` (`:47`) via `microsoft-cognitiveservices-speech-sdk`, SSML voice name in `<voice name="...">` (`:52`), output hardcoded to `Audio48Khz96KBitRateMonoMp3` (`:48`), `WordBoundary` tuples with escaped-entity correction (`:56` to `:66`), spacing `REQUEST_SPACING_MS = 3100` (`:10`) for the F0 20-per-minute limit. Dead config: `tts.mjs` never reads `brand.tts.outputFormat` or `brand.tts.rate` from `brand.json` though both exist, and there is no `mstts` wrapper. The orchestrator passes `key: process.env.AZURE_SPEECH_KEY, region: process.env.AZURE_SPEECH_REGION` for the narrator (Brand B's `tools/publish.mjs:142`); both repos' `.dev.vars` carry the same shared F0 key. `r2-upload.mjs` shells to `wrangler r2 object put` with a 4-attempt backoff around an intermittent libuv abort on Windows (`:12` to `:37`), cache profiles `IMMUTABLE` (`:39`) and `SHORT` (`:40`), helpers `putJson`/`putAudio`/`putImage` (`:42` to `:53`), manifest last with SHORT (`publish.mjs:262`, `putJson(..., false)`). State default `{ chapters: {}, libraryVersion: 0, charLedger: {} }` (`:53` to `:55`), `state.charLedger[month] += charCount` (`:152`), hard stop `MONTHLY_CHAR_BUDGET = 450_000` (`:32`, `:134` to `:140`). Live values: Brand B's state file `charLedger {"2026-07": 95085}`, `libraryVersion 5`; Brand A's state file `charLedger {"2026-07": 27503}`, `libraryVersion 6`. The F0 resource is shared, so real usage is the sum (about 122,588 this month) and the per-brand ledgers do not reconcile it. Bucket naming follows `<reader-app>-<brand>-content`; `contentHash(obj) = sha256(JSON.stringify(obj)).slice(0,8)` (`tools/lib/md.mjs:201`).

**Q1 drift proof.** Brand B's `publish.mjs` is ahead on voice: `--force` (`:14`, `:83` `args.force || prev?.hash !== hash`) and a voice-aware audio hash `contentHash({ blocks, title, voice: brand.tts.voice })` (`:160`) stored as `audio: { durationMs, hash: audioHash }` (`:171`); live proof Brand B's state file shows a prologue chapter with content hash `f58735e1` but audio hash `533b31d4`. Brand A's `publish.mjs` is behind on voice but ahead on sort metadata: no `--force` (`:82` `const changed = prev?.hash !== hash`), audio keyed by the plain content hash (`:160`), manifest emits `timeframe: book.timeframe` (`:233`) which Brand B lacks; live proof Brand A's state file story-01 audio hash `4eeaaeb4` equals its content hash (content-keyed, not voice-aware), story-02 shows content `2e286ccf` vs audio `39400593`. So Brand A (the 42-story catalog, the bulk of the MAI voice backfill) runs the older non-voice-aware code, and reconciliation is a two-way merge.

**Q2 owner-decided specifics.** The same listen set ships for both brands: Harper (F, en-US), Lisa (F, en-AU), Ethan (M, en-US, `excited` style, chosen because his natural register reads flat). Example entry `{ "id": "en-US-Ethan:MAI-Voice-2", "slug": "ethan", "label": "Ethan", "gender": "m", "style": "excited" }`, added in four files (`brands/<brand>/brand.json` in both repos). Region `eastus`. The `maiLedger` `estUsd = chars * 22 / 1_000_000`; owner cap is 100 USD per month (supersedes an earlier 15 USD proposal), enforced only after the owner's Azure credit resets; until then run uncapped to burn credit. App-side hook is in Brand B's `app/src/lib/player.ts` `startPlayback` (`:118` to `:137`, `audioController.load(contentUrl(chapter.audio), content, timings, ...)`, `setReadAlongMode` at `:157`), with `NowPlayingItem` (`:18` to `:26`) carrying the variants.
...
**Q3 image locations.** Brand A images live in the site repo (Brand A's reader-app repo) under `public/images/`: `covers/` (`story-NN.png`, about 1264x848), `stories/` (`story-NN-scene-*.png`), `illustrations/` (`illus-*.png`, 1024x1024), plus `brand/`, `hero/`, `social/`, `ui/`. Story frontmatter carries `coverImage: "/images/covers/story-01.png"` and `artStyle: "graphite"` or `"colored-pencil"`; scene art is inline body markdown, for example `![alt](/images/stories/story-01-scene-01.png)` (`src/content/stories/story-01.md:10`, `:39`, `:47`). Covers resolve via `publish.mjs` `coverSourceFor` (`:181` to `:216`); scenes are rewritten by `tools/lib/md.mjs` `resolveSrc` (`:59` to `:63`, `:115`) to `siteOrigin` (Brand A's site origin), and `blockPlainText` returns `''` for image blocks (`:190`). The 1264x848 covers (1,071,872 px) exceed the MAI 1,048,576-pixel cap, so the nearest valid 3:2 landscape is 1248x832. The prompt library is in a separate repo (the studio prompt repo, `resources/`: `Illustration_Prompts_All_Stories.md`, `Branding_Illustration_Prompts.md`, `Character_Bible.md`); the reader-app repo has no `resources/` directory, so a plan citing a `resources/` path in the reader-app repo is stale. The proposed tool is `tools/mai-image.mjs` in Brand A's reader-app repo; genericize trademarked terms (for example drop the named apparel brand) in prompts per SPIKE-04. Secrets by name in `kv-<workload>-<env>-01`.

**Q5 concrete file list.** Voice, in both Brand B's and Brand A's publish pipelines: `tools/publish.mjs`, `tools/tts.mjs`, `brands/<brand>/brand.json` in each repo (four files total), `app/src/lib/{types,state,player,downloads}.ts`, `app/src/screens/{NowPlaying,Settings}.tsx`, `docs/content-pipeline.md`; runtime `tools/.state/<brand>.json`. No change: `tools/stitch.mjs`, `tools/r2-upload.mjs`, `app/src/reader/AudioController.ts`, `app/src/reader/SpeechFallback.ts`, the worker. Image, Brand A only: new `tools/mai-image.mjs`, `public/images/{stories,covers}/*.png`, `public/images/provenance.json`, `src/content/stories/*.md` frontmatter (all in Brand A's reader-app repo); read-only prompt source `resources/Illustration_Prompts_All_Stories.md` in the studio prompt repo.
...
- Sibling spikes: `docs/research/SPIKE-04-identity-security.md` (auth, RBAC, secret names, responsible AI), `ai/plans/source/ai-voice-mai-voice-2.md`, `ai/plans/source/mai-image-2-5-art-match.md`
