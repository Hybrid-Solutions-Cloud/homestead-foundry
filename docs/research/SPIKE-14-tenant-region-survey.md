...
Date: 2026-07-22
Scope: build a model-by-region availability matrix for the full current-and-planned roster on the model roster (MAI-Image-2.5, the FLUX family, MAI-Voice-2, and the gpt-5.6-terra / grok-4-1-fast-reasoning reviewer pair) and confirm whether East US still hosts every one of them. Region availability for preview models changes over time, so this spike re-verifies the region facts directly against the Microsoft Learn catalog rather than trusting the region findings in `docs/research/SPIKE-01-image-model.md` and `docs/research/SPIKE-02-voice-model.md` as still current.

Grounding documents read first: the model roster (the authoritative roster), `docs/adr/ADR-0004-foundry-topology-and-region.md` (the topology-and-region decision this spike tests), `docs/research/SPIKE-01-image-model.md`, and `docs/research/SPIKE-02-voice-model.md`.
...
- In-repo, re-verified against the above: `docs/research/SPIKE-01-image-model.md` (image RPM ladder, Tier 5 = 10 RPM in East US), `docs/research/SPIKE-02-voice-model.md` (MAI voice East US and preview-styles flag), `docs/adr/ADR-0004-foundry-topology-and-region.md` (the East US topology decision this spike confirms), the model roster (the authoritative roster).
