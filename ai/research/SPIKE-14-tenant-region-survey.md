...
Date: 2026-07-22
Scope: build a model-by-region availability matrix for the full current-and-planned roster on `pmo/BACKLOG.md` (MAI-Image-2.5, the FLUX family, MAI-Voice-2, and the gpt-5.6-terra / grok-4-1-fast-reasoning reviewer pair) and confirm whether East US still hosts every one of them. Region availability for preview models changes over time, so this spike re-verifies the region facts directly against the Microsoft Learn catalog rather than trusting the region findings in `ai/research/SPIKE-01-image-model.md` and `ai/research/SPIKE-02-voice-model.md` as still current.

Grounding documents read first: `pmo/BACKLOG.md` (the authoritative roster), `ai/adr/ADR-0004-foundry-topology-and-region.md` (the topology-and-region decision this spike tests), `ai/research/SPIKE-01-image-model.md`, and `ai/research/SPIKE-02-voice-model.md`.
