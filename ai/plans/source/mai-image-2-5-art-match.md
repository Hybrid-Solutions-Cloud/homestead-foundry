...
**Task:** G11 (new Story #1 scene art for Brand A's reader app, and future stories), serving the Brand A / Brand B reader-app brands.

### Recon findings (the original Brand A art)

Files inspected read-only in Brand A's reader-app repo:
...
- **Trademark:** the character tokens specify a named apparel brand's overalls with its logo patch. Genericizing to "denim bib overalls with brass buttons and a red rectangular chest patch" removes the risk without changing the look.
...
- Key Vault: **`kv-<workload>-<env>-01`**; suggested secret name `<workload>-mai-image-key`.

### Local sources

- Brand A's reader-app repo: `resources/Illustration_Prompts_All_Stories.md`, `src/content/stories/*.md` frontmatter, `public/images/covers/` and `public/images/stories/`
