...
- Companion diagrams (`ai/diagrams/INDEX.md`): **#10 Deployment runbook** (this guide's steps 2 through 8 in dependency order) and **#9 Tenant selection and fallback decision** (this guide's section 10)
...
- Record the outcome of every step (real names, chosen version strings, role assignments, secret names by name only, budget configuration) for `ai/implementation/as-built.md`, which is written when deployment completes.
...
- `ai/design/resource-topology-and-caf-naming.md` (canonical names, tags, topology, deployment shape, reuse rules)
- `ai/design/identity-and-security.md` (auth models per surface, RBAC table, secret rules, network posture, security checklist)
- `ai/design/cost-and-governance.md` (three enforcement layers, budget spec, tag inheritance, measurement plan, cost checklist)
- `ai/design/reliability-and-operations.md` (version pinning and the periodic re-check, operating rhythm, failure modes, fallback ordering)
- `ai/design/architecture-overview.md`, `ai/design/performance-efficiency.md`, `ai/design/pipeline-integration-design.md` (endpoints, canvas, pacing, ledger sidecar)
- `ai/adr/ADR-0001-target-tenant.md` (primary and fallback, three pre-checks, regional-availability blocker pattern)
- `ai/adr/ADR-0002-image-model-and-access.md` (version re-query, retiring siblings, canvas, cost probe)
- `ai/adr/ADR-0003-voice-model-and-voice-set.md` (no voice deployment, voice set, spike questions)
- `ai/adr/ADR-0004-foundry-topology-and-region.md` (kind, SKU, subdomain, two-resource split)
- `ai/adr/ADR-0005-identity-and-secrets.md` (Entra for image, key for Speech, role table, secret naming)
- `ai/adr/ADR-0006-cost-governance.md` (budget thresholds, action group, spending limit ON, tag scheme)
- `ai/adr/ADR-0007-content-safety-and-responsible-ai.md` (prompt hygiene applied to the smoke test)
- `ai/adr/ADR-0008-publish-pipeline-integration.md` (pre-render invariants; nothing at runtime depends on Azure)
- `ai/research/SPIKE-03-tenant-readiness.md` (fallback pre-check commands and reasoning, credit mechanics)
- `ai/verification/environment-readiness.md` (precondition baselines, budget-before-spend prerequisite)
- `ai/diagrams/INDEX.md` (diagrams #9 and #10)
...
**Real outcome:** this runbook was executed in full through Gate 8 for both Gunner the Lab and Holdfast Press; `ai/implementation/as-built.md` records the completed run, and the deployment has been held per Gate 8 pending owner-authorized bulk generation.
