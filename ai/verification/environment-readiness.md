...
Scope actually checked: can this build deploy one Azure AI Foundry (AIServices) resource in region East US that hosts a MAI-Image-2.5 model deployment (Global Standard) and serves MAI-Voice-2 through Azure Speech, in the owner's preferred subscription first, falling back to the azurelocal.cloud tenant if not.

### Summary and go/no-go

**GO**, in the primary subscription. This Is My Demo - MVP Subscription (tenant This Is My Demo) is ready today: the resource provider is registered, an AIServices resource in Standard (S0) tier is offerable in East US, MAI-Image-2.5 appears in the live model catalog for East US in Preview status with a Global Standard deployment path, MAI-Voice-2 is confirmed for East US on Microsoft Learn's Azure Speech region table, quota headroom exists on every meter checked, and no policy assignment blocks Cognitive Services resource creation.

No fallback deployment is needed right now. If the primary subscription becomes unusable (credit exhaustion, access change), the azurelocal.cloud tenant has one candidate subscription that is ready with the same caveats as the primary (azlz-cmp-lz-core-001) and one that needs a provider-registration step first (azurelocalcloud-prodtech-eus-management). Both are documented below for that scenario.

### Candidate subscriptions (by name)

| Subscription | Tenant | Role | State |
|---|---|---|---|
| This Is My Demo - MVP Subscription | This Is My Demo | Primary (current CLI default) | Enabled |
| azlz-cmp-lz-core-001 | azurelocal.cloud tenant | Fallback candidate 1 (preferred of the two) | Enabled |
| azurelocalcloud-prodtech-eus-management | azurelocal.cloud tenant | Fallback candidate 2 | Enabled |
...
|---|---|
| This Is My Demo - MVP Subscription | **Registered** |
| azlz-cmp-lz-core-001 | **Registered** |
| azurelocalcloud-prodtech-eus-management | **NotRegistered** |

If azurelocalcloud-prodtech-eus-management is ever needed, `az provider register -n Microsoft.CognitiveServices` must run first and be allowed to finish propagating before any resource create is attempted there.
...
| Meter | Scope | This Is My Demo - MVP Subscription | azlz-cmp-lz-core-001 | azurelocalcloud-prodtech-eus-management |
|---|---|---|---|---|
...
|---|---|---|
| This Is My Demo - MVP Subscription | 1: an ASC (Microsoft Defender for Cloud) initiative for open-source relational database protection | No, unrelated resource type |
| azlz-cmp-lz-core-001 | 1: "ASC Default," Defender for Cloud's built-in security initiative | No, audit/monitor only, not a deny effect on resource creation |
| azurelocalcloud-prodtech-eus-management | 0 | No blocker visible at this scope |

None of the assignments found carry a deny effect against Cognitive Services or AIServices resource types. Caveat for the azurelocal.cloud tenant specifically: this is a subscription-scoped query only. Azure landing zone architectures (and the subscription names here, `azlz-cmp-lz-core-001`, follow that naming convention) commonly apply guardrail policies at the management group level, which can still apply to a subscription without showing up as a subscription-scoped assignment in every case. Nothing found here contradicts a go, but a management-group-scoped policy check (or simply attempting a real deployment) is the only way to fully rule this out for the azurelocal.cloud tenant. This caveat does not apply to the primary subscription's recommendation.

### Existing Cognitive Services accounts (avoid duplication)

| Subscription | Existing account | Kind | Tier | Location | Resource group |
|---|---|---|---|---|---|
| This Is My Demo - MVP Subscription | storyreader-tts | SpeechServices | F0 | eastus | rg-storyreader |
| azlz-cmp-lz-core-001 | azl-ai-demo-resource | AIServices | S0 | eastus2 | rg-azl-ai-demo |
| azurelocalcloud-prodtech-eus-management | none | | | | |

Two useful facts fall out of this:

1. **storyreader-tts cannot host the new work.** It is kind `SpeechServices` (single-service), not `AIServices`, so it cannot take a Foundry model deployment like MAI-Image-2.5. It is also F0 tier, and the voice plan already flags F0 eligibility for MAI-Voice-2 as unverified. A new resource is required regardless of what the spike finds; this is confirmation, not new risk.
2. **azl-ai-demo-resource proves AIServices resources deploy successfully in the azlz-cmp-lz-core-001 subscription**, just in eastus2 rather than eastus today. That is real precedent for the fallback path, on top of the clean provider/quota/policy checks above.
...
2. **A resource group** in This Is My Demo - MVP Subscription, region East US (both source plans suggest names; neither exists yet, confirmed by this check's resource group listing).
...
5. **Auth decision carried over from the source plans**: Entra ID (Cognitive Services Contributor role, `DefaultAzureCredential`) is preferred with no stored key; if a key is used instead, it goes to Key Vault kv-hcs-vault-01 by name only, never into a committed file.
...
No further read-only checks were needed to move forward; the blockers, if any turn out to exist, are the management-group policy caveat noted above for the azurelocal.cloud tenant, which only matters if the fallback is ever activated.
