...
- `ai/adr/ADR-0006-cost-governance.md` (three-layer enforcement, budget spec, tag keys, credit mechanics, EA-only credit alerts)
- `ai/adr/ADR-0002-image-model-and-access.md` (tokens-per-image UNKNOWN, smoke-test probe, Flash lever, retiring alternatives)
- `ai/adr/ADR-0001-target-tenant.md` (MVP credit subscription, spending limit behavior)
- `ai/adr/ADR-0003-voice-model-and-voice-set.md` (22 USD per 1M characters, S0, every MAI character billable)
- `ai/research/SPIKE-05-cost-governance.md` (rate card and its confidence caveats, rollups, budget latency facts, tag mechanics, reporting substitutes)
- `ai/research/SPIKE-01-image-model.md` (token mechanics and the measurement method, via SPIKE-05)
- `ai/research/SPIKE-06-pipeline-integration.md` (the existing F0-tier character guard the pipeline ledger mirrors, verified in `publish.mjs` in both consumer repos)
