...
- `ai/adr/ADR-0008-publish-pipeline-integration.md` (Option A, reconciliation first, `--voices` and variant hashing, R2 variant keys, `audioVariants`, `mai-image.mjs`, prompt source, provenance index)
- `ai/adr/ADR-0003-voice-model-and-voice-set.md` (voice set, expressive style via `mstts:express-as`, listen-only v1, spike items)
- `ai/adr/ADR-0002-image-model-and-access.md` (model, version re-check, 1248x832 canvas, no seed, candidate review, Flash arm)
- `ai/adr/ADR-0005-identity-and-secrets.md` and `ai/adr/ADR-0007-content-safety-and-responsible-ai.md` (auth and prompt hygiene, via the companion designs)
- `ai/research/SPIKE-06-pipeline-integration.md` (as-is pipeline, drift, hook points, file lists)
...
**Concrete Azure names** (canonical for this instance, cross-referenced by the other `ai/design/` docs before their own genericization passes):
