...
- `ai/adr/ADR-0002-image-model-and-access.md` (canvas math, parameter surface, tier ladder and Tier 6 ceiling, Flash sequencing, cost-probe design)
- `ai/adr/ADR-0003-voice-model-and-voice-set.md` (voice set, S0 and per-request limits, 22 USD per 1M characters)
- `ai/adr/ADR-0008-publish-pipeline-integration.md` (pre-render architecture, hashes, `--voices`, variant keys, ledger and guard placement, rejection of on-demand synthesis)
- `ai/adr/ADR-0004-foundry-topology-and-region.md` (S0 200 TPS default)
- `ai/adr/ADR-0006-cost-governance.md` (cost bands and rollups the throughput math reuses; guard defaults)
- `ai/research/SPIKE-01-image-model.md` (canvas table, RPM ladder, prompt-token context, 680-call phrasing noted in the Worked example)
- `ai/research/SPIKE-02-voice-model.md` (10-minute cap, 64 KB SSML, S0 TPS, F0 pacing origin)
- `ai/research/SPIKE-05-cost-governance.md` (catalog character counts, per-voice costs, 340-image basis, R2 free-tier fit)
- `ai/research/SPIKE-06-pipeline-integration.md` (3.1-second spacing location, per-voice options bag, cache profiles)
